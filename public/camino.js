(() => {
  ...
  function map(entry) {
    ...
    return `<div class="public-route-panel"><div class="public-tour" data-public-tour="true" data-public-tour-id="${escape(entry.id)}"><canvas class="public-route-canvas" role="img" aria-label="${escape(entry.title)}"></canvas><div class="public-map-loading" data-tour-loading role="status">${escape(t("mapLoading"))}</div><small class="public-map-attribution">${escape(t("attribution"))}</small><div class="public-tour-controls"><button type="button" data-tour-play aria-label="${escape(t("play"))}" title="${escape(t("play"))}"></button><input type="range" min="0" max="1000" value="0" step="1" data-tour-progress aria-label="${escape(t("play"))}"></div></div></div>`;
  }
  ...
  function mountPublicTours() {
    ...
    import("/diary-3d.js?v=34").then(({ mountDiaryTour }) => {
      ...
      const mount = (tour) => {
        ...
        mounted.set(tour, mountDiaryTour(tour, entry, { play:t("play"), pause:t("pause"), replay:t("replay"), error:t("mapError") }));
      };
      const unmount = (tour) => {
        mounted.get(tour)?.();
        mounted.delete(tour);
