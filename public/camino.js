(() => {
  ...
  Object.assign(copy.de, { follow:"Kamera folgt dem Wanderer", fullscreen:"Karte maximieren", exitFullscreen:"Vollansicht beenden", pace:"Tempo" });
  Object.assign(copy.en, { follow:"Camera follows the walker", fullscreen:"Maximise map", exitFullscreen:"Exit fullscreen", pace:"Speed" });
  Object.assign(copy.ru, { follow:"Камера следует за путником", fullscreen:"Развернуть карту", exitFullscreen:"Выйти из полноэкранного режима", pace:"Темп" });
  let language = "de";
  ...
  function elevation(entry) {
    ...
    const speedRange = speedLine ? `${format(Math.min(...profile),1)}–${format(Math.max(...profile),1)} km/h` : "";
    const legend = `<div class="public-elevation-legend" style="display:flex;justify-content:space-between;gap:.5rem;margin:0 0 .35rem;color:#61716d;font-size:.58rem;font-weight:750"><span><i style="display:inline-block;width:.55rem;height:.55rem;margin-right:.25rem;border-radius:50%;background:#e3ae3b"></i>${escape(t("elevation"))} <b style="color:#17312f">${format(entry.stats?.min)}–${format(entry.stats?.max)} m</b></span>${speedLine ? `<span><i style="display:inline-block;width:.55rem;height:.55rem;margin-right:.25rem;border-radius:50%;background:#287ca0"></i>${escape(t("pace"))} <b style="color:#17312f">${speedRange}</b> · Ø ${format(entry.stats.averageSpeed,1)} km/h</span>` : ""}</div>`;
    const marker = `<span class="public-elevation-marker-halo" data-elevation-marker aria-hidden="true" style="position:absolute;width:18px;height:18px;border-radius:50%;transform:translate(-50%,-50%);background:rgba(232,74,42,.2);pointer-events:none"></span><span class="public-elevation-marker" data-elevation-marker aria-hidden="true" style="position:absolute;width:9px;height:9px;border:2px solid #fff7e8;border-radius:50%;transform:translate(-50%,-50%);background:#e84a2a;pointer-events:none"></span>`;
    return `<div class="public-elevation-wrap">${legend}<div class="public-elevation-plot" style="position:relative"><svg class="public-elevation" data-elevation-profile viewBox="0 0 640 72" preserveAspectRatio="none" role="slider" tabindex="0" aria-label="${escape(t("elevation"))}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><path d="${elevationPath(entry.track)}"></path>${speedLine}</svg>${marker}</div></div>`;
  }
